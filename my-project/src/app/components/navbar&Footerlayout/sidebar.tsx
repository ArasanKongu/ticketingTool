...
const SideBar = () => {
	const [sidebarOpen, setSidebarOpen] = useState(
		typeof localStorage !== 'undefined' && localStorage.getItem(sideBarOpenState)
			...
			default:
				return (
					<Link
						href={menu.link}
						key={menu.name}
						className={`flex items-center justify-start font-medium h-7 rounded-md hover:bg-background hover:text-primary ${sidebarOpen ? " pl-1 w-[8rem] " : " justify-center w-[1.75rem] "
							}  ${isActive
					...
					>
						<div className="flex flex-row items-center gap-x-1">
							...
							<span
								className={`flex items-center justify-center text-xs overflow-hidden truncate ${!sidebarOpen && "hidden"
									}`}
								style={{
									overflow: "hidden",
	...
	return (
		<div className="hidden md:flex z-10 top-0 sticky">
			<div
				className={`scrollbar-none ${sidebarOpen ? "w-36" : "w-12 relative"
			...
			>
				...
				>
					<IoIosArrowBack
						className={`w-[1rem] h-[1rem] text-primary-foreground justify-center ml-1 cursor-pointer ${!sidebarOpen && "rotate-180"
							}`}
					/>
				</button>
			</div>
